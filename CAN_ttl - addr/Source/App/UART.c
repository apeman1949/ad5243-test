...
void UART1_WriteBytes(unsigned char  *cmd_buf,unsigned char  offset,unsigned char n )
{
	int i=0,end;
	end=n+offset;
 ...
 if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)
	{ 	
	  c=USART2->DR;  
		...
		//index_uart2=index_uart2%uart_buffer_size2;
	} 
...
unsigned short CheckCRC(unsigned char *pData,unsigned int siLen)
{
  ...
  return siRet;
	...
	u16 CrcCal(u8 *data,u8 index,u8 num)//????,??
{     u8 i,j,con1,con2;   
...
void USART3_IRQHandler(void)                    //??3??????
{
  unsigned char  c;
	...
	{ 	
	    c=USART3->DR;
		 //UART1_WriteByte(c);
//	  if(sensor_init_en==13)
//		{
//  	  UART1_WriteByte(c);
//  		return;
//		}
		
		uart_rec_buffer3[index_uart3++]=c;	//将收到的数据存入缓冲区中
	   if (uart_rec_buffer3[0]!=0x55) //数据头不对，则重新开始寻找0x55数据头
	    {
		    index_uart3=0;
		     return;
	     }
	if (index_uart3<11) {return;}//数据不满11个，则返回
	   else
	   {
		    switch(uart_rec_buffer3[1])//判断数据是哪种数据，然后将其拷贝到对应的结构体中，有些数据包需要通过上位机打开对应的输出后，才能接收到这个数据包的数据
		     {
			     case 0x50:	memcpy(&stcTime,&uart_rec_buffer3[2],8);break;//memcpy为编译器自带的内存拷贝函数，需引用"string.h"，将接收缓冲区的字符拷贝到数据结构体里面，从而实现数据的解析。
			     case 0x51:	memcpy(&stcAcc,&uart_rec_buffer3[2],8);break;
			     case 0x52:	memcpy(&stcGyro,&uart_rec_buffer3[2],8);break;
			     case 0x53:	memcpy(&stcAngle,&uart_rec_buffer3[2],8);break;
			     case 0x54:	memcpy(&stcMag,&uart_rec_buffer3[2],8);break;
			     case 0x55:	memcpy(&stcDStatus,&uart_rec_buffer3[2],8);break;
			     case 0x56:	memcpy(&stcPress,&uart_rec_buffer3[2],8);break;
			     case 0x57:	memcpy(&stcLonLat,&uart_rec_buffer3[2],8);break;
			     case 0x58:	memcpy(&stcGPSV,&uart_rec_buffer3[2],8);break;
			     case 0x59:	memcpy(&stcQ,&uart_rec_buffer3[2],8);break;
		     }
		index_uart3=0;//清空缓存区 	  
	  }
  }
